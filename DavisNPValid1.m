...
perms = logspace(-9,7,NumSteps);
vfs = 0.01; %[0.05,0.1,0.15,0.2,0.3];

basefnm = ""+baseFolder+"/QESIM_S41_C" + runCase + "_VF"
...
for j = 1:NumSteps
    for i = 1:length(vfs)
        ...
        runLine = "matlab -r "+char(39)+"QESIM_Exp_SNP(90, 1, "+ vfs(i) +", 40E-9, [10, 1], [1E-7,"+thisPerm+"], 10000, "+runID+")"+char(39)+"\n";
    
        fnms((i-1)*NumSteps+j) = basefnm +""+ i + "_"+ j +".pbs"
        fid = fopen(fnms((i-1)*NumSteps+j),'wt');
        fprintf(fid, "# QE NL_COMSOL Compare V41 - C"+runCase+"  Nano Copper Fillers\n");
        fprintf(fid, "#PBS -N QESIM41_NP_C"+runCase+"_VF"+i+"\n");
...
% write shell script
fid = fopen(""+baseFolder+"/RunJobs41_CV_Case"+runCase+".sh",'wt');
